...
export default function LandlordContractors() {
	const {
		states,
		cities,
		zipCodes,
	...
	const [landlordContractorFilters, setLandlordContractorFilters] = useState([
		{
			name: 'areasServed',
			label: 'Areas Served',
	...
	React.useEffect(() => {
		fetchBusinessTags().then((tags) => {
			...
			const updatedContractorFilters = [
				...landlordContractorFilters,
				businessTagsOptions,
			];
			setLandlordContractorFilters(updatedContractorFilters);
		});
	}, []);
	...
	React.useEffect(() => {
		dispatch(
			searchProfiles(
				`marketplaceProfiles/search/?type=landlord-contractor&${currentUrlParams.toString()}`
			)
		);
	...
	return (
		<MarketplaceSearch
			...
			profileTypeGroup='landlord-contractors'
		>
			<Stack direction='row' mb={2} gap='1em' flexWrap='wrap'>
				...
				{landlordContractorFilters?.map((filter, i) => (
					<SuiSelect
						key={i}
