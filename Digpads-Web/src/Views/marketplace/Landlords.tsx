...
import { searchProfiles } from 'features/marketplaceProfile/marketplaceProfileSlice';

export default function Landlords() {
	const { states, cities, zipCodes, selectedCity, onChange } = useStates();
	...
	React.useEffect(() => {
		fetchBusinessTags().then((tags) => {
			...
			const updatedLandlordFilters = [...landlordFilters, businessTagsOptions];
			setLandlordFilters(updatedLandlordFilters);
		});
	}, []);
	...
	React.useEffect(() => {
		dispatch(
			searchProfiles(
				`marketplaceProfiles/search/?type=landlord&${currentUrlParams.toString()}`
			)
		);
