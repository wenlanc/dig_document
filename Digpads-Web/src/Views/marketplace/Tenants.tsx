...
export default function Tenants() {
	...
	const [tenantFilters, setTenantFilters] = useState([
		...
		},
		{
			name: 'desiredAreas',
			label: 'Desired Areas',
			options: [{ label: 'Area #1', value: 'Area #1' }],
		},
		{
			name: 'desiredRentals',
			label: 'Desired Rentals',
			options: usaCities.map((city) => ({
				label: `${city.city}(${city.state})`,
				value: `${city.city}, ${city.state}`,
			})),
	...
	React.useEffect(() => {
		fetchBusinessTags().then((tags) => {
			...
			const updatedContractorFilters = [...tenantFilters, businessTagsOptions];
			setTenantFilters(updatedContractorFilters);
		});
	}, []);
	...
	React.useEffect(() => {
		dispatch(
			searchProfiles(
				`marketplaceProfiles/search/?type=tenant&${currentUrlParams.toString()}`
			)
		);
